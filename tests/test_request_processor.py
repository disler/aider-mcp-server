...
async def test_register_handler(request_processor):
    ...
    async def test_handler(request_id: str, transport_id: str, request_data: dict) -> OperationResult:
        return {"success": True, "request_id": request_id, "transport_id": transport_id, "result": {"message": "Test Result"}}

    await request_processor.register_handler("test_operation", test_handler, Permissions.EXECUTE_AIDER)
...
async def test_process_request(request_processor):
    ...
    assert request_processor._active_requests["123"]["operation_name"] == "test_operation"
