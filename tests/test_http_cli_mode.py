...
def run_main_http(
    ...
    try:
        ...
        if mock_serve_http.side_effect and \
           isinstance(mock_serve_http.side_effect, Exception) and \
           e is mock_serve_http.side_effect:
...
def test_http_mode_default_params(monkeypatch: pytest.MonkeyPatch):
    ...
    (
        mock_serve_http, _, _, mock_path_is_dir_instance_check, mock_is_git_repo, mock_exit, _,
        mock_path_resolve, mock_path_exists_instance_check
...
def test_http_mode_custom_params(monkeypatch: pytest.MonkeyPatch):
    ...
    args = [
        "--server-mode", "http",
        "--http-host", custom_host,
        "--http-port", str(custom_port),
        "--current-working-dir", "."
    ]
    (
        mock_serve_http, _, _, _, mock_is_git_repo, mock_exit, _, mock_path_resolve, _
    ) = run_main_http(monkeypatch, args)
...
def test_http_mode_working_dir_not_exists(monkeypatch: pytest.MonkeyPatch):
    ...
    assert f"Error: Specified working directory does not exist: {non_existent_dir}" in mock_logger_instance.critical.call_args[0][0]
...
def test_http_mode_working_dir_not_git_repo(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    (
        mock_serve_http, _, mock_logger_instance, _, mock_is_git_repo, mock_exit, _, _, _
    ) = run_main_http(monkeypatch, args)
...
def test_http_mode_invalid_port_range(monkeypatch: pytest.MonkeyPatch, invalid_port: int):
    ...
    args = [
        "--server-mode", "http",
        "--http-port", str(invalid_port),
        "--current-working-dir", "."
    ]
    (
        mock_serve_http, _, mock_logger_instance, _, _, mock_exit, _, _, _
    ) = run_main_http(monkeypatch, args)
...
def test_http_mode_port_conflict(monkeypatch: pytest.MonkeyPatch):
    ...
    (
        mock_serve_http, _, mock_logger_instance, _, _, mock_exit, mock_asyncio_run, _, _
    ) = run_main_http(monkeypatch, args)
...
def test_http_mode_verbose_logging(monkeypatch: pytest.MonkeyPatch):
    ...
    (
        mock_serve_http, mock_get_logger, mock_logger_instance, _, _, mock_exit, _, _, _
    ) = run_main_http(monkeypatch, args)
