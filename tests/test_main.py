...
def mock_get_logger() -> Generator[MagicMock, None, None]:
    ...
    with patch("aider_mcp_server.__main__.get_logger", autospec=True) as mock_get_logger_factory:
        ...
        # Ensure all methods used in __main__ are mocked and will be tracked
...
def test_invalid_cwd_not_git_repo(
    ...
    with patch.object(Path, "resolve", return_value=mock_resolved_path):
        ...
        with patch(
            "aider_mcp_server.__main__.is_git_repository",
            return_value=(False, git_error_msg),
        ) as mock_git_check:
            with patch("aider_mcp_server.__main__.get_logger") as mock_get_logger:
