    ...
    with patch('aider_mcp_server.sse_transport_adapter.FastMCP') as mock_fastmcp_class:
        ...
        with patch.object(adapter, 'logger') as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with(
                "No coordinator available, FastMCP will not be initialized"
            )
...
async def test_adapter_integrates_with_mcp_transport(mock_mcp_transport, mock_fastmcp):
    ...
    # Create the adapter with the mock coordinator
    with patch('aider_mcp_server.sse_transport_adapter.FastMCP', return_value=mock_fastmcp), \
    ...
    with patch('starlette.responses.Response') as mock_response_cls:
        ...
        with patch.object(adapter, 'logger') as mock_logger:
            ...
            mock_logger.error.assert_called_once_with(
                "SSE transport or MCP server not initialized"
            )
            
            # Verify that a 500 error response was returned
            mock_response_cls.assert_called_once_with(
                "Server not properly initialized", status_code=500
            )
            assert response == mock_response
...
async def test_aider_ai_code_tool_integration():
    ...
    with patch('aider_mcp_server.sse_transport_adapter.process_aider_ai_code_request') as mock_process_request, \
...
async def test_list_models_tool_integration():
    ...
    with patch('aider_mcp_server.sse_transport_adapter.process_list_models_request') as mock_process_request, \
         patch('aider_mcp_server.sse_transport_adapter.FastMCP') as mock_fastmcp_class, \
         patch.object(adapter, 'logger') as mock_logger:
...
async def test_tool_error_handling():
    ...
    with patch('aider_mcp_server.sse_transport_adapter.process_aider_ai_code_request',
