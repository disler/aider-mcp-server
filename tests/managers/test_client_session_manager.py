...
class TestClientSessionManager:
    ...
    async def test_update_session_activity_nonexistent(self, manager: ClientSessionManager, caplog):
        ...
        assert f"Attempted to update activity for non-existent client session: {client_id}" in caplog.text
        assert client_id not in manager._active_sessions
    ...
    async def test_cleanup_expired_sessions(self, manager: ClientSessionManager, tmp_path: Path):
        ...
        with patch("aider_mcp_server.managers.client_session_manager.CLIENT_SESSION_TIMEOUT", short_timeout):
            # Register an active client (will update its activity)
            await manager.register_client(client_id_active)
            
            # Register a client that will expire
            session_expired = await manager.register_client(client_id_expired)
            # Manually set its last_activity far back enough to ensure it's considered expired
            # Or rely on sleep, but this is more deterministic for short_timeout
            session_expired.last_activity = datetime.now(timezone.utc) - timedelta(seconds=short_timeout * 2)
            manager._active_sessions[client_id_expired] = session_expired # ensure the manager has this modified session

            await asyncio.sleep(short_timeout / 2) # Wait a bit, but less than timeout

            # Update activity for the active client so it doesn't expire
            await manager.update_session_activity(client_id_active)

            await asyncio.sleep(short_timeout) # Wait for the other session to definitely be expired
    ...
    async def test_cleanup_expired_sessions_already_cleaned_up(self, manager: ClientSessionManager, caplog):
        ...
        with patch("aider_mcp_server.managers.client_session_manager.CLIENT_SESSION_TIMEOUT", short_timeout):
            ...
            expected_log = (
                f"Session for client {client_id} was already cleaned up "
                "during expired session sweep."
            )
