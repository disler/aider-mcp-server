...
async def test_run_sse_server_initialization_and_shutdown():
    ...
    with patch('aider_mcp_server.sse_server.ApplicationCoordinator.getInstance', 
               return_value=asyncio.Future()) as mock_get_instance, \
         patch('aider_mcp_server.sse_server.SSETransportAdapter', 
               return_value=mock_adapter) as mock_adapter_class, \
        ...
        await run_sse_server(
            host="127.0.0.1",
            port=8765,
            current_working_dir="/mock/git/repo"
        )
...
async def test_adapter_handle_queue_full_during_shutdown():
    ...
    adapter._active_connections = {
        "normal-connection": asyncio.Queue(),
        "full-connection": full_queue
    }
    ...
    with patch("asyncio.sleep", new_callable=AsyncMock), \
        ...
        # Verify that a debug log was made about the full queue
        mock_logger.debug.assert_any_call(mock_logger.debug.call_args_list[0][0][0])
