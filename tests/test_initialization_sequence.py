...
class TestInitializationSequence(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.get_logger_patcher = patch(
            "aider_mcp_server.initialization_sequence.get_logger", mock_get_logger_global
        )
        ...
        self.app_coordinator_patcher = patch(
            "aider_mcp_server.initialization_sequence.ApplicationCoordinator", autospec=True
        )
        ...
        self.wait_for_patcher = patch("aider_mcp_server.initialization_sequence.asyncio.wait_for")
    ...
    async def test_initialize_transport_registration_fails(self):
        ...
        self.mock_coordinator_instance.register_transport.assert_awaited_once_with(
            "faulty_transport", config="bad"
        )
        self.mock_coordinator_instance.shutdown.assert_awaited_once() # Attempt cleanup
        self.assertFalse(self.init_seq._initialized)
        mock_logger_instance.error.assert_any_call(
