...
class TestChangesIntegration:
    ...
    async def test_integration_empty_file_creation(
        self, mock_configure_model, mock_setup_coder, mock_run_aider
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            orig_process = _process_coder_results
            process_calls = []
            ...
            async def mock_process(*args, **kwargs):
                ...
                file_status = {"has_changes": True, "status_summary": "Changes detected: 1 files created (new_file.py)", "files_created": 1, "files_modified": 0}
            ...
            with patch(
                "aider_mcp_server.atoms.tools.aider_ai_code._process_coder_results", mock_process
            ):
                # Create a test case where an empty file is created
                test_file = os.path.join(temp_dir, "new_file.py")

                # Setup result that mimics no meaningful changes but file creation
                file_content = "File contents after editing (git not used):\n\n--- new_file.py ---\n# Empty file\n"
                empty_summary = summarize_changes(file_content)
                empty_status = {"has_changes": False, "status_summary": "No changes detected.", "files_created": 0, "files_modified": 0}
                ...
                with patch("aider_mcp_server.atoms.tools.aider_ai_code._check_for_meaningful_changes") as mock_check:
                    ...
                    with patch("aider_mcp_server.atoms.tools.aider_ai_code.get_file_status_summary") as mock_status:
                        ...
                        assert mock_run_aider.return_value["success"] is False, "Original result should have success=False"
    ...
    @patch("aider_mcp_server.atoms.tools.aider_ai_code._setup_aider_coder")
    @patch("aider_mcp_server.atoms.tools.aider_ai_code._configure_model")
    async def test_integration_with_changes_summary(
        self, mock_configure_model, mock_setup_coder, mock_run_aider
    ):
...
"""
            ...
            with patch("aider_mcp_server.atoms.tools.aider_ai_code.get_changes_diff_or_content") as mock_diff:
                ...
                mock_status = {"has_changes": True, "status_summary": "Changes detected: 0 files created, 1 files modified.", "files_created": 0, "files_modified": 1}
                ...
                with patch(
                    "aider_mcp_server.atoms.tools.aider_ai_code.summarize_changes"
                ) as mock_summarize:
                    mock_summary = {
