...
async def test_initialize_with_no_coordinator():
    ...
    with patch.object(adapter, "_initialize_fastmcp") as mock_init_fastmcp, \
...
async def test_server_start_error():
    ...
    async def mock_start_listening():
        ...
        # Configure server
        config = MagicMock()  # Mock uvicorn.Config
    ...
    with patch.object(adapter, "start_listening", side_effect=mock_start_listening), \
         patch.object(adapter, "logger") as mock_logger:
