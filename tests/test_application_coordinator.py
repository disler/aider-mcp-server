...
# It's better to patch 'aider_mcp_server.application_coordinator.get_logger'
...
class TestApplicationCoordinator(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.get_logger_patcher_ac = patch('aider_mcp_server.application_coordinator.get_logger', mock_get_logger)
        self.get_logger_patcher_es = patch('aider_mcp_server.event_system.get_logger_func', mock_get_logger) # EventSystem uses get_logger_func
        # EventCoordinator takes logger_factory parameter, no direct import to patch
        self.get_logger_patcher_rp = patch('aider_mcp_server.request_processor.get_logger', mock_get_logger)
        self.get_logger_patcher_tar = patch('aider_mcp_server.transport_adapter_registry.get_logger_func', mock_get_logger) # TAR uses get_logger_func
        self.get_logger_patcher_hr = patch('aider_mcp_server.handler_registry.get_logger', mock_get_logger)
        ...
        self.event_system_patcher = patch('aider_mcp_server.application_coordinator.EventSystem', autospec=True)
        self.event_coordinator_patcher = patch('aider_mcp_server.application_coordinator.EventCoordinator', autospec=True)
        self.request_processor_patcher = patch('aider_mcp_server.application_coordinator.RequestProcessor', autospec=True)
        self.transport_registry_patcher = patch('aider_mcp_server.application_coordinator.TransportAdapterRegistry', autospec=True)
        self.handler_registry_patcher = patch('aider_mcp_server.application_coordinator.HandlerRegistry', autospec=True)
    ...
    async def test_register_transport(self):
        ...
        # Per Task 9 spec, initialize_adapter is called with (name, **kwargs)
        self.mock_transport_registry.initialize_adapter.assert_called_once_with("test_transport", **kwargs)
    ...
    def test_register_handler_class(self):
        ...
        class DummyHandlerClass:
            async def handle_op1(self, request): return {}
            async def handle_op2(self, request): return {}
        ...
        self.mock_handler_registry.get_handler.side_effect = lambda type: mock_op1_handler if type == "op1" else None
