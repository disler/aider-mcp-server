...
async def test_coordinator_event_propagation_to_adapter():
    ...
    with patch('aider_mcp_server.transport_adapter.AbstractTransportAdapter.initialize', 
               new=AsyncMock()):
        ...
        original_send_event = adapter.send_event
...
async def test_run_sse_server_with_invalid_working_dir():
    ...
    with patch('aider_mcp_server.sse_server.is_git_repository', 
              return_value=(False, "Not a git repository")) as mock_git_check:
        ...
        with pytest.raises(ValueError) as excinfo:
            await run_sse_server(
                host="127.0.0.1",
                port=8765,
                current_working_dir="/invalid/dir"
            )
