...
from aider_mcp_server.atoms.types.streaming_types import AiderChangesSummary, FileChangesSummary, ChangeType
...
async def test_changes_summary_complexity_estimation():
    ...
    response = {
        ...
        "changes_summary": {
            ...
            "files": [
                {"name": f"src/module_{i}.py", "operation": "modified"} for i in range(12)
            ],
            "stats": {
                "lines_added": 250,
                "lines_removed": 180,
...
async def test_changes_summary_category_detection():
    ...
    for summary_text, expected_categories in test_cases:
        ...
        for category in expected_categories:
            assert category in payload["change_categories"], f"Expected {category} in {payload['change_categories']} for '{summary_text}'"
...
async def test_changes_summary_error_handling():
    ...
    response = {
        "success": True,
        "changes_summary": {"summary": "Test changes"},
        "file_status": {}
    }
