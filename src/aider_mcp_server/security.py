                ...
                if isinstance(perm, Permissions):
                    ...
                    if enum_perm:
                        processed_permissions.add(enum_perm)
                         ...
                         logger.warning(f"Granting all permissions due to wildcard '*' in context for user '{user_id or 'anonymous'}'.")
...
def create_context_from_credentials(credentials: Dict[str, Any], transport_id: Optional[str] = None) -> SecurityContext:
    ...
    logger.debug(f"Attempting to create security context from credentials (keys: {cred_keys}) for transport '{transport_id}'")
    ...
    if auth_token and isinstance(auth_token, str):
        ...
        if auth_token == "VALID_TEST_TOKEN":
            ...
            granted_permissions: Set[Union[Permissions, str]] = {
                Permissions.EXECUTE_AIDER, Permissions.LIST_MODELS, Permissions.VIEW_CONFIG
            }
    ...
    logger.debug(f"No valid credentials found or processed for transport '{transport_id}', returning anonymous context.")
