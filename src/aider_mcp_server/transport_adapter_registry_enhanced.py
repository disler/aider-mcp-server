...
class EnhancedTransportAdapterRegistry:
    ...
    def discover_adapters(self, package_name: str) -> None:
        ...
        recommended = sum(1 for meta in self._transport_metadata.values() if meta.status == TransportStatus.RECOMMENDED)
        deprecated = sum(1 for meta in self._transport_metadata.values() if meta.status == TransportStatus.DEPRECATED)
    ...
    def _analyze_transport_capabilities(self, adapter_class: Type[AbstractTransportAdapter]) -> TransportCapabilities:
        ...
        if hasattr(adapter_class, "MCP_PROTOCOL_VERSION"):
            version_str = adapter_class.MCP_PROTOCOL_VERSION
            if version_str == "2025-03-26":
                capabilities.protocol_version = TransportProtocolVersion.MCP_2025_03_26

        if hasattr(adapter_class, "SUPPORTS_AUTHORIZATION"):
            capabilities.supports_authorization = bool(adapter_class.SUPPORTS_AUTHORIZATION)

        if hasattr(adapter_class, "DEPRECATED"):
            capabilities.deprecated = bool(adapter_class.DEPRECATED)
    ...
    def _determine_transport_status(self, capabilities: TransportCapabilities) -> TransportStatus:
        ...
        if (
            capabilities.protocol_version == TransportProtocolVersion.MCP_2025_03_26
            and capabilities.supports_authorization
            and capabilities.supports_bidirectional
        ):
    ...
    async def get_transport_recommendations(self) -> Dict[str, Any]:
        ...
        """
        async with self._lock:
            recommendations: Dict[str, Any] = {"recommended": [], "supported": [], "deprecated": [], "rationale": {}}
            ...
            for transport_type, metadata in self._transport_metadata.items():
                ...
                if metadata.capabilities.deprecated:
                    transport_info["deprecation_message"] = (
                        metadata.capabilities.deprecation_message or "Transport is deprecated"
                    )
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            return {
                ...
                "transport_breakdown": {
                    status.value: sum(1 for meta in self._transport_metadata.values() if meta.status == status)
