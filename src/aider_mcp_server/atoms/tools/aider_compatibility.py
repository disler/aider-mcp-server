...
def get_version_specific_params(aider_version_str: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        version.parse(aider_version_str)  # Just check if it parses, don't need the value
...
def check_aider_compatibility(expected_version: str) -> None:
    ...
    if actual < expected:
        raise AssertionError(f"Installed aider version {actual_version} is older than expected {expected_version}")
