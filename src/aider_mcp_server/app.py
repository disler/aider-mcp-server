...
def create_app(
    ...
    async def sse_endpoint(request: Request) -> Response:
        ...
        if not _adapter:
            ...
            # Return 503 Service Unavailable if adapter isn't ready
            return Response(content="Server not ready", status_code=503, media_type="text/plain")

        # Check if the coordinator (accessed via adapter) is shutting down
        if _adapter._coordinator and _adapter._coordinator.is_shutting_down():
            logger.warning("SSE connection attempt rejected: Server is shutting down.")
    ...
    # Route for tool call messages (or other messages)
