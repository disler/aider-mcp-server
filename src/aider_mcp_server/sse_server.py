...
async def _aider_ai_code_handler(request_id: str, transport_id: str, parameters: Dict[str, Any], security_context: SecurityContext) -> Dict[str, Any]:
    """Handler for aider_ai_code operation."""
    return {"success": True}

async def _list_models_handler(request_id: str, transport_id: str, parameters: Dict[str, Any], security_context: SecurityContext) -> Dict[str, Any]:
    ...
    try:
        ...
        async with coordinator:
            ...
            [
                ...
                Route("/message", endpoint=lambda request: JSONResponse({"status": "ok"}), methods=["POST"]),
            ]
        ...
        for sig_num, _handler in sync_signal_handlers.items():
            try:
                if loop.is_running() and not loop.is_closed():
