...
@asynccontextmanager
async def coordinator_shutdown_context(
...
async def _register_handlers(coordinator: ApplicationCoordinator, editor_model: str, cwd_path: Path) -> None:
    ...
    await coordinator.register_handler("list_models", list_models_wrapper)
    ...
    try:
        while not shutdown_event.is_set() and server_task in pending:
            ...
            if server_task in done_part and not server_task.cancelled():
                ...
                if exc:
                    logger.error(
                        f"Task {server_task.get_name()} exited with exception:",
                        exc_info=exc,
                    )
                else:
...
async def _shutdown_uvicorn_server(
    server: uvicorn.Server, server_task: asyncio.Task[Any], done: Set[asyncio.Task[Any]]
) -> None:
...
async def serve_multi_transport(
    ...
    logger.info(
        f"Starting multi-transport server with config: host={host}, port={port}, editor_model={editor_model}, cwd={cwd_path}, heartbeat={heartbeat_interval}s"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Host for SSE (default: {DEFAULT_WS_HOST})",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=DEFAULT_WS_PORT,
        help=f"Port for SSE (default: {DEFAULT_WS_PORT})",
    )
    parser.add_argument(
        "--editor-model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="SSE heartbeat interval (sec) (default: 15.0)",
    )
