...
class HttpServerManager:
    ...
    async def create_client_session(self, client_request: ClientRequest) -> SessionInfo:
        ...
        async with self._lock:
            ...
            self.logger.info(
                f"Created client session: {session_id} for client {client_id} "
                f"with server {server_id}"
            )
    ...
    async def destroy_client_session(self, client_id: str) -> None:
        ...
        async with self._lock:
            ...
            self.logger.info(
                f"Destroyed client session: {session_info.session_id} for client {client_id}"
            )
